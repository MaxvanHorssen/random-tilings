from ._weight_aztec import weight_aztec,weight_aztec_forceE
from ._weight_hexagon import weight_hexagon
from ._core import reduce_weight,reduce_weight_hd,clear_CTower,shuffling,shuffling_hd,reduce_weight_forceE
from ._draw_dominos import draw_dominos
from ._draw_lozenges import draw_lozenges
from numpy import round,array2string,ndarray,round,ascontiguousarray,int64
...
def Aztec(n,w,gap=False,hard_drive_mode=False):
    ...
    if isinstance(w.dtype,(float,int)):
        w = w.astype(float)
        W = weight_aztec(n,w)
        dtype = 0
    else :
        W = weight_aztec_forceE(n,w)
        dtype = 1
    if type(gap)==ndarray:
        N = 2*n
        end = N
    ...
    if hard_drive_mode:
        ...
        if dtype == 0:
            ...
            W,E = ascontiguousarray(W.real),ascontiguousarray(W.imag.astype(int64))
            C,E,of = reduce_weight_forceE(W,E)
        if of and config.warnings:
            print('Numerical instability detected: Overflow/Underflow!')
