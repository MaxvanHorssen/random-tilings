...
@njit("(int8,Array(int8, 2, 'C', False, aligned=True),bool,bool,bool)",cache=True)
def compute_domino_data(n,M,diamond,ac_gray,paths):
    N = 2*n
    if paths:
        P = zeros((n*(n+1),2,2))
    else:
...
def draw_dominos(n,M,gap=False,color_scheme='standard',dot_width=0,dpi=100,edge_width=0,
    ...
    ac_gray = str(color_scheme).lower()=='aztec gray'
    ...
    #-----------------------------------------------------------
    P,C = compute_domino_data(n,M,diamond,ac_gray,path_width>0)
    ...
    if path_width > 0:
        if edge_width > 0:
            P0,_ = compute_domino_data(n,M,diamond,False,False)
